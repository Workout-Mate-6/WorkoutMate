...
| --- | --- | --- |
| **장점** | -  추가 인프라 불필요 (기존 DB 활용)   

- SQL만으로 구현 가능

- 트랜잭션/ACID 보장으로 데이터 정합성 높음 | - 조회수(score) 자동 정렬 지원

- 메모리 기반 O(logN) → 빠른 성능

- TTL 기반 자동 만료 관리- DB 부하 분산 (API는 Redis만 조회)

- 확장성 높음 (대규모 트래픽 대응 유리) |
  | **단점** | - 데이터 증가 시 집계 쿼리 성능 저하

- 매 요청마다 집계 쿼리 실행 → DB 부하↑

- 실시간 반영 시 DB 성능에 영향 | - Redis 메모리 사용량 관리 필요
